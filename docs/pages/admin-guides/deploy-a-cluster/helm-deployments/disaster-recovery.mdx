...
for information about using KMS keys in multiple regions. Note that KMS support
is only possible in the `teleport-cluster` Helm chart using the
`auth.teleportConfig` values field ([chart
...
1. Once the Teleport Auth Service is running on the new cluster, make sure that
   ...
   Teleport Kubernetes operator can manage them. You should manage your Teleport
   resources as a set of Kubernetes manifests applied using a GitOps tool like
   Flux CD so that, when you launch the new cluster, you can readily apply them.
...
1. Teleport Agents can remain connected to your previous Teleport cluster. For
   any Agents running outside the region affected by the disaster, plan to
   restart them once you have changed the DNS record for your Teleport cluster
   to point to your newly redeployed Proxy Service. If you have deployed Agents
   in the affected region, you will need to redeploy them to the new region.
...
If a Teleport admin makes an update to a record on the cluster state backend
after the last backup, that update will not be reflected on the backend when
restoring it during a disaster recovery procedure.

For dynamic resources, you can prevent the cluster state backend in the
unavailable region from falling out of sync with the backup in the new region by
configuring dynamic resources using infrastructure as code. In this setup,
dynamic resource configurations exist in a source repository managed with a
version control tool.  An automated system ensures that every update to your
dynamic resource configuration triggers an update on the cluster state backend.
This way, you can reapply all dynamic resources against the new cluster state
backend without missing any updates.

You should also impose a change freeze for dynamic resources during the disaster
recovery procedure. Lock any Teleport roles that include permissions to edit
dynamic resources.

Teleport roles allow access to modifying dynamic resources with the
`spec.allow.rules` field. You can query your Teleport cluster for all roles with
these permissions using your JSON parsing tool of choice. For example, you can
use the following `jq` command to list all roles with one or more rule that
grants permissions to modify backend resources:

```code
$ tctl get roles --format json | jq -r '.[] | select(.spec.allow.rules | length > 0) | .metadata.name'
dashboard-admin
dashboard-user
device-admin
device-enroll
...
Create a lock using a `tctl lock` command, e.g.:

```code
$ tctl lock --role=contractor --message="Change freeze during disaster recovery" --ttl=3h
```

Teleport manages access to certificate authority rotations using the same
role-based access controls it offers for dynamic resources. In this case, the
resource name is `cert_authority`. A rotation requires the following
permissions:
...
Note that, at this point, any Machine & Workload Identity bots that have joined
the cluster with a static token will likely miss their renewal periods and
become locked out of the cluster. We recommend using delegated join methods to
