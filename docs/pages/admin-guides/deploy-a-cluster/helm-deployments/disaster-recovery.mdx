...
{/*TODO Hugo: "AFAIK, Unlike what its name says, "tctl get all" doesn't get call
resources in the backend. I'm not familiar with dynamo but I would expect many
resources to go missing."

ResourceCommand.GetAll is defined in `tool/tctl/common/resource_command.go`. It
ranges over the result of `services.GetResourceMarshalerKinds`, which is a
slice of keys of `resourceMarshalers`. We populate `resourceMarshalers` in
`RegisterResourceMarshaler` (`lib/services/resource.go`). We register resource
marshalers in `init` (lib/services/resource.go).

TODO: May want to generate a list of `tctl get all` resources and create a
partial. We can include that in the backup/restore guide too.*/}
...
backend. You can use S3 replication rules to create continuous backups from your
...
{/*TODO: do you need to maintain a continuous backup of session recordings? What
about backing up to another solution (near-online or offline), then applying the
backup to a new S3 bucket?/*}
