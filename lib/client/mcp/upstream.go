...
// upstream is an MCP client to an upstream MCP server over TLS tunnel.
type upstream struct {
	mcpclient.MCPClient
}

// Upstream implements the MCPClient interface using stdio communication.
...
type Upstream struct {
	...
	responses     map[int64]chan mcpclient.RPCResponse
	mu            sync.RWMutex
	done          chan struct{}
	initialized   bool
...
func newUpstream(r io.ReadCloser, w io.WriteCloser) (*Upstream, error) {
	u := &Upstream{
		stdin:     w,
		stdout:    bufio.NewReader(r),
		responses: make(map[int64]chan mcpclient.RPCResponse),
...
func (c *Upstream) OnNotification(
...
func (c *Upstream) sendRequest(
	...
	responseChan := make(chan mcpclient.RPCResponse, 1)
	c.mu.Lock()
...
func (c *Upstream) Ping(ctx context.Context) error {
	_, err := c.sendRequest(ctx, "ping", nil)
	return err
}

func (c *Upstream) Initialize(
	ctx context.Context,
...
func (c *Upstream) ListResourceTemplates(
	ctx context.Context,
