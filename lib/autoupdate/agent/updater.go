...
func NewLocalUpdater(cfg LocalUpdaterConfig, ns *Namespace) (*Updater, error) {
	...
	return &Updater{
		...
		ReexecSetup: func(ctx context.Context, pathDir string, enableSELinux, reload bool) error {
			...
			if enableSELinux {
				args = append(args, "--install-selinux")
			} else {
				args = append(args, "--no-install-selinux")
			}
			if reload {
...
type OverrideConfig struct {
	...
	// SELinuxFlagSet specifies whether the user explicitly toggled SELinux behavior.
	SELinuxFlagSet bool
}
...
func (u *Updater) update(ctx context.Context, cfg *UpdateConfig, target Revision, force, agpl bool) error {
	...
	revertConfig := func(ctx context.Context) bool {
		...
		if err := u.SetupNamespace(ctx, cfg.Spec.Path, cfg.Spec.EnableSELinux, false); err != nil {
	...
	if cfg.Status.Active != target {
		err := u.ReexecSetup(ctx, cfg.Spec.Path, cfg.Spec.EnableSELinux, true)
		if errors.Is(err, context.Canceled) {
		...
		err := u.ReexecSetup(ctx, cfg.Spec.Path, cfg.Spec.EnableSELinux, false)
		if errors.Is(err, context.Canceled) {
			return trace.Errorf("check canceled")
		}
		if err != nil {
...
func (u *Updater) Setup(ctx context.Context, path string, installSELinux, restart bool) error {
	...
	if !installSELinux {
		...
		removeSELinux = cfg.Spec.EnableSELinux
	}
