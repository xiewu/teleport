...
type AuthHandlerConfig struct {
	...
	// OnGitRBACFailure is used by the git forwarder for events/metrics.
	OnGitRBACFailure func(conn ssh.ConnMetadata, ident *sshca.Identity, err error)
}
...
func (h *AuthHandlers) UserKeyAuth(conn ssh.ConnMetadata, key ssh.PublicKey) (ppms *ssh.Permissions, rerr error) {
	...
	if err != nil {
		...
		if h.c.Component == teleport.ComponentForwardingGit && h.c.OnGitRBACFailure != nil {
			h.c.OnGitRBACFailure(conn, ident, err)
		}
	...
	if proxyPermit != nil {
		...
		permissions.Extensions[utils.ExtIntProxyingPermit] = string(encodedPermit)
	}
	...
	if gitForwardingPermit != nil {
		...
		permissions.Extensions[utils.ExtIntGitForwardingPermit] = string(encodedPermit)
	}
